...
from dyrep_node_month import DyRepNode_month
...
def extract_months_from_timestamps(timestamps):
    months = []
    for ts in timestamps:
        # 텐서를 numpy 배열로 변환
        ts_in_seconds = ts.item() / 1000.0  # 밀리초를 초로 변환
        # 타임스탬프를 datetime 객체로 변환
        dt_object = datetime.fromtimestamp(ts_in_seconds)
        # datetime 객체에서 월(month) 정보 추출
...
if __name__ == '__main__':
    ...
    # 모델 설정
    model = DyRepNode_month(num_nodes=train_set.N_nodes,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, data_batch in enumerate(tqdm(train_loader)): #tqdm이 막대그래프 표시하는 역할
            ...
            time_cur = data_batch[3]

            months = torch.tensor(extract_months_from_timestamps(time_cur))
            print(months)

            change_points = []
            # 이전 값을 첫 번째 요소로 초기화
            previous_value = months[0]

            # 리스트의 두 번째 요소부터 순회
            for i in range(1, len(months)):
                current_value = months[i]
                # 값이 변하는 지점 찾기
            ...
            if not change_points:
                pred_prob, pred_time = model(data_batch, 0, batch_size-1)
            
            else:
                change_point_prev = 0
                for changing_point in change_points:
                    pred_prob, pred_time = model(data_batch , change_point_prev, changing_point)
                    last_pred_prob = pred_prob[-1]
            ...
            mae_prob = loss_mae(y_pred,y_true)
        ...
        # loss 리스트에 추가
        total_loss = float(total_loss)/386
        total_mae = float(total_mae)
        total_cross = float(total_cross)
        ...
        mae, cross_entropy, test_ap, test_auc = test_all(model, test_reoccur_time_hr, args.device ,args.test_batch_size)
        
        #리스트에 정보 추가
        all_test_mae.append(mae)
        all_test_loss.append(cross_entropy)
        all_test_ap.append(test_ap)
        all_test_auc.append(test_auc)

        print("epoch {}/{}".format(epoch+1, args.epochs + 1))
        print("Train: loss {:.5f}, total_mae {:.5f}, total_cross {:.5f}, time per batch {:.5f}".format(
            total_loss, total_mae, total_cross, time_iter/float(batch_idx+1)))

        print('Test: mae={:.5f}, cross_entropy {:.5f}, ap {:.5f}, auc{:.5f}\n\n'.format(
            mae, cross_entropy, test_ap, test_auc))
    ...
    plt.plot(np.arange(1, args.epochs + 1), np.array(total_maes), 'r', label='total maes')
