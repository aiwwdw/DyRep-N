...
def test_all(model, return_time_hr, device, batch_size):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(test_loader)):
            ...
            maes += model.mae(y_pred, y_true)
            y_pred_thres = [1 if score >= 0.5 else 0 for score in y_pred]
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # loss 리스트에 추가
        total_loss = float(total_loss)/train_batch_num
        # total_mae = float(total_mae)
        total_cross = float(total_cross)/train_batch_num
